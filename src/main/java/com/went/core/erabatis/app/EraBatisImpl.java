...
import com.went.core.erabatis.component.function.Table;
...
public class EraBatisImpl implements EraBatis {
    ...
    private void registerEntityEntry(EntityEntry entityEntry) {
        ...
        TableSource table = entityEntry.getTable();
        entityColumns.put(entityClass, bindWithTable((Table) table, unmodifiable(entityEntry.getFields())));
        entityTables.put(entityClass, table);
        entityPks.put(entityClass, bindWithTable((Table) table, unmodifiable(entityEntry.getPks())));
    }


    private Collection<Field> unmodifiable(Collection<Field> fields) {
    ...
    public Field getColumnByAlias(Class entityClass, String alias) {
        Map<String, Field> entityColumns = aliasMap.get(entityClass);
        try {
            return entityColumns.get(alias);
    ...
    public QueryAction select(Class entity) {
        Collection fields = entityColumns.get(entity);
        return selectStatement().select(fields).from(entityTables.get(entity));
    }
    ...
    @Override
    public QueryAction updateStatement() {
        return null;
    }
